 ...
 * @return string
 */
function theme_ridestur_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_ridestur', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_ridestur and not theme_boost (see the line above).
    ...
    $pre = file_get_contents($CFG->dirroot . '/theme/ridestur/scss/pre.scss');
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    $post = file_get_contents($CFG->dirroot . '/theme/ridestur/scss/post.scss');
 ...
 * @param $settingname
 */
function theme_ridestur_update_settings_images($settingname) {
    global $CFG;

    // The setting name that was updated comes as a string like 's_theme_ridestur_loginbackgroundimage'.
